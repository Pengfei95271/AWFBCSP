"""
CSP/FBCSP/AWFBCSP + 传统分类器噪声鲁棒性分析 (BCIC-IV-2A数据集)
...
print("=" * 80)
print("🛡️ CSP/FBCSP/AWFBCSP + 传统分类器噪声鲁棒性分析 (BCIC-IV-2A数据集)")
print("=" * 80)
...
class Config:
    ...
    # 结果保存路径
    results_dir = 'results/csp_traditional_classifiers_noise_robustness'
...
for noise_level in config.noise_levels:
    ...
    if not noise_data.empty:
        ...
        print(f"{noise_level*100:6.0f}%    {best_result['Feature']:<10} {best_result['Classifier']:<20} "
              ...
              f"A{best_result['Best_Subject']:02d}        A{best_result['Worst_Subject']:02d}")
...
ax3.set_title('噪声水平 vs 特征提取方法 性能热力图')
...
if robustness_data:
    ...
    ax4.set_ylabel('性能下降 (%)')
    ax4.set_title('噪声鲁棒性指标 (0% vs 30%噪声)')
