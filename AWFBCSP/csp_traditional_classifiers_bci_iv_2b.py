"""
CSP/FBCSP/AWFBCSP + 传统分类器性能对比实验 (BCIC-IV-2B数据集)
...
print("=" * 80)
print("🧠 CSP/FBCSP/AWFBCSP + 传统分类器性能对比实验 (BCIC-IV-2B数据集)")
print("=" * 80)
...
class Config:
    ...
    # 结果保存路径
    results_dir = 'results/csp_traditional_classifiers_bci_iv_2b'
...
def load_bci_iv_2b_data():
    ...
    for subject_id in config.subjects:
        ...
        if os.path.exists(train_data_path) and os.path.exists(train_label_path):
            ...
            print(f"   被试 {subject_name}: {train_data.shape[0]} trials, 标签: {np.unique(train_labels)}")
        else:
...
for i, (_, row) in enumerate(results_df_sorted.head(15).iterrows(), 1):
    print(f"{i:<4} {row['Feature']:<10} {row['Classifier']:<20} "
          ...
          f"B{row['Best_Subject']:02d}        B{row['Worst_Subject']:02d}")
...
for subject_id in config.subjects:
    for feature_name in feature_extractors.keys():
        if subject_id in subject_results and feature_name in subject_results[subject_id]:
            ...
            print(f"B{subject_id:02d}    {feature_name:<10} {clf_name:<20} "
...
ax1.set_title('特征提取方法性能对比 - BCIC-IV-2B\n(个体准确率, 3通道)', fontweight='bold')
...
ax2.set_title('分类器性能对比 - BCIC-IV-2B\n(个体准确率, 3通道)', fontweight='bold')
...
ax3.set_title('特征提取方法 vs 分类器 性能热力图\nBCIC-IV-2B (3通道)', fontweight='bold')
ax3.set_xlabel('分类器')
...
for subject_id in config.subjects:
    for feature_name in feature_extractors.keys():
        if subject_id in subject_results and feature_name in subject_results[subject_id]:
            ...
            subject_labels.append(f"B{subject_id:02d}\n{feature_name}")
...
if subject_accuracies:
    ...
    ax4.set_title('个体被试最佳性能分布 - BCIC-IV-2B', fontweight='bold')
...
print(f"   最佳被试: B{best_result['Best_Subject']:02d}")
print(f"   最差被试: B{best_result['Worst_Subject']:02d}")
